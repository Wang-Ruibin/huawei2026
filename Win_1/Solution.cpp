...
using namespace std;
const double EPS = 1e-7; // 些许提升，可以再看看
...
bool SegmentsStrictIntersect(const Vector2D &a, const Vector2D &b,
...
bool check(const Polygon &P, const Polygon &Q, const Vector2D &vec)
{
    ...
    // 2. 检查边严格相交
    for (auto &e : edge)
        if (SegmentsStrictIntersect(e.a, e.b, e.c + vec, e.d + vec))
            return false;
    edge.clear();
    ...
    for (int i = 0; i < nP; ++i)
    {
        ...
        for (int j = 0; j < nQ; ++j)
        {
            ...
            if (SegmentsStrictIntersect(a, b, c, d))
            {
                edge.push_back({a, b, c - vec, d - vec});
                // 存在严格内部交点 → 重叠
            }
        }
    }
    ...
    if (PointInPolygonStrict(vP[0], Q))
        return false;
    if (PointInPolygonStrict(vQ[0], P))
        return false;
...
Vector2D GenSolution(const Vector2D &vec)
{
    ...
    if (projA.max <= projB.min || projB.max <= projA.min)
        return {0.0, 0.0};
    minOverlap = std::min(projA.max - projB.min, projB.max - projA.min);
