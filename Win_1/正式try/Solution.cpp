...
using namespace std;
const double EPS = 1e-6; // 些许提升，可以再看看
...
double GenSolution(const Vector2D &vec)
{
    ...
    // 指数扩张：完全不依赖 minOverlap
    while (true)
    {
        Vector2D vect = vec * maxn;
    ...
    while (maxn - minn >= EPS)
    {
        double mid = (maxn + minn) * 0.5;
        Vector2D vect = vec * mid;
        Polygon polyans = polyB;
        polyans.MoveByVec(vect);
...
void PreProcess()
{
    ...
    // 随机起始角度（0 到 2π）
    std::mt19937 gen(0);
    std::uniform_real_distribution<double> angleGen(0.0, 2.0 * M_PI);
    ...
    {
        g_escapeDist[i] = GenSolution(g_escapeDirs[i].Normalize());
        g_escapeDirs[i] = g_escapeDirs[i].Normalize() * g_escapeDist[i];
    }
}
