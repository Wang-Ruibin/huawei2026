...
double calOverlap(const Projection &projA, const Projection &projB)
{
    double overlap = 0;
    ...
    else if ((projA.min >= projB.min && projA.max <= projB.max) || (projB.min >= projA.min && projB.max <= projA.max))
        overlap = std::min(std::max(projA.max, projB.max) - std::max(projA.min, projB.min), std::min(projA.max, projB.max) - std::min(projA.min, projB.min));
    else
        overlap = std::min(projA.max, projB.max) - std::max(projA.min, projB.min);
    return overlap;
}
...
Vector2D GenSolution(const Vector2D &vec)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        for (size_t j = 0; j < currentPoly.vertices.size(); ++j)
        {
            ...
            // Projection projA = ProjectPolygon(polygon1, axis); // 可以判断下限
            ...
            double overlap = calOverlap(projA, projB);
            ...
            for (size_t i = 2; i < std::max(polygon1.vertices.size(), polyB.vertices.size()); ++i)
            {
                ...
                overlap = calOverlap(projA, projB);
            }
...
int main()
{
    freopen("practice_7.in", "r", stdin);
    freopen("practice_7.out", "w", stdout);
    // =============== 1. read polygons ===================
    cin >> n1 >> n2;
